...
const BalaklavaForm: React.FC<BalaklavaFormProps> = ({ formData, handleChange }) => {
    console.log('BalaklavaForm - Current form data:', formData); 
    ...
    return (
        <div className="p-4 md:p-6" style={{ fontFamily: '"Comic Sans MS", cursive' }}>
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6 md:gap-10 lg:gap-20 px-4">
                    {[
                        { value: 'A', label: 'Strings', imgSrc: '/img/balaklava.png' },
                        { value: 'B', label: 'Open Face', imgSrc: '/img/cotton.png' },
                        { value: 'C', label: 'Eyes Only', imgSrc: '/img/eyes.png' },
                        { value: 'D', label: 'Animal Ears', imgSrc: '/img/muse.png' },
                        { value: 'E', label: 'Net Style', imgSrc: '/img/netbala.png' },
                        { value: 'F', label: 'Scrappy', imgSrc: '/img/scrapybala.png' },
                    ].map((bala) => (
                        <label key={bala.value} className="flex flex-col items-center cursor-pointer transition-transform hover:scale-105 p-4">
                            <input
                                type="radio"
                                name="type"
                                value={bala.value}
                                checked={formData.type === bala.value}
    ...
    <Image
        ...
        className={`
            ...
            ${formData.type === bala.value ? 'border-pink-600 bg-pink-100 shadow-xl' : ''}
        `}
        priority
    />
</div>
                        ...
                        return (
                            <label key={yarn} className="cursor-pointer transition-transform hover:scale-105 mr-2 sm:mr-4 relative">
                                <input
                                    ...
                                    className="hidden"
                                />
                                <span className="text-pink-700 font-bold text-xl sm:text-3xl">
                                    {yarn}
                                </span>
                                {isSelected && (
            ...
            <div className="mb-6">
                ...
                <input type="text" name="width" value={formData.measurements.width} onChange={handleChange} className="w-1/3 py-2 px-3 border-2 border-pink-300 rounded-full focus:outline-none focus:ring-2 focus:ring-pink-200 text-gray-700 font-semibold" required />
